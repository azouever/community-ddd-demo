...
/**
 * 该类中属性命名不规范，目的是为了表述translator组件存在的意义。 请忽略不规范命名，实际项目中，强烈推荐model和entity的属性名保持一致，如果能做到这一点，完全可以不要translator和entity。
 * 直接使用model进行持久化操作。
 *
 * @author daoqidelv
...
public class PostEntity extends BaseEntity {
  ...
  /**
   * 发帖时间 NOTE： posting_time 和 BaseEntity.create_time不一样，posting_time是domain层赋值，在业务上表示发帖时间，而create_time是数据模型层的概念，和domain无关。
   */
